...
using SMSRateLimitingMS.Application.UseCases.GetCurrentStatus;
...
namespace SMSRateLimitingMS.API.Controllers
{
    ...
    public class MonitoringController(IMediator _mediator, ILogger<MonitoringController> _logger) : ControllerBase
    {
        ...
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(IEnumerable<CurrentStatusDto>))]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError, Type = typeof(string))]
        ...
        public async Task<IActionResult> GetCurrentStatus(CancellationToken cancellationToken)
        {
            try
            {
                var result = await _mediator.Send(new GetCurrentStatusQuery(), cancellationToken);

                return Ok(result);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting rate limit current status");
                return StatusCode(StatusCodes.Status500InternalServerError, "An error occurred while getting rate limit current status");
            }
        }
    }
}
